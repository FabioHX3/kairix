#!/usr/bin/env python3
"""
Script para adicionar submenu mobile na página configurar.html
"""
...
# 2. Substituir o link "Soluções" pelo submenu
# Buscar padrões possíveis
OLD_PATTERNS = [
    '<li><a href="#products" onclick="closeMobileMenu()">Soluções</a></li>',
    '<li><a href="/#products" onclick="closeMobileMenu()">Soluções</a></li>',
    '<li><a href="/" onclick="closeMobileMenu()">Soluções</a></li>'
]
...
for pattern in OLD_PATTERNS:
    if pattern in conteudo:
        conteudo = conteudo.replace(pattern, HTML_SUBMENU)
        print(f'  ✅ HTML do submenu substituído (padrão: {pattern[:30]}...)')
        encontrou = True
        break

if not encontrou:
    print('  ⚠️  Padrão do HTML não encontrado, tentando buscar...')
...
print('\n\n✅ Submenu mobile adicionado ao painel de configuração!')
